...
#
# Usage:
#   scripts/synth_cctv_look.sh <input.mp4> <output.mp4> [cam_label] [base_epoch]
#
...
if [[ $# -lt 2 ]]; then
  echo "usage: $0 <input.mp4> <output.mp4> [cam_label] [base_epoch]" >&2
  exit 2
fi
...
echo "  label='${cam_label}' base_epoch=${base_epoch} dur=${dur}s frames=${seconds}"
...
eq=saturation=0.55:contrast=1.12:brightness=-0.03:gamma=0.94,\
gblur=sigma=0.5,\
